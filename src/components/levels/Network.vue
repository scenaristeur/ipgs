...
export default {
  ...
  methods:{
    ...
    async explorePod(src){
      if (this.useSource == true ){
        ...
        // console.log(dataset)
        let pos = {x:0, y:0}
        this.$setCurrentThingUrl({url: src, pos: pos})
      }
    ...
    async exploreFriends(webId){
      ...
      this.data.nodes.update([n])
      let pos  = this.network.getPositions(n.id)[n.id]
      ...
      //
      //
      //   let nf = { id: f.webId, label: f.webId, x:pos.x, y:pos.y}
      //   nodes.push(nf);
      ...
      if (pod.storage != undefined){
        let pubNode = {
          id: pod.storage+'public/',
          label: pod.storage+'public/',
          x: pos.x,
          y: pos.y
        }
        nodes.push(n)
        let ed = {from: pod.webId, to: pubNode.id, label: "publicStorage"}
        this.data.edges.add([ed])
        this.$setCurrentThingUrl({url: pod.storage+'public/', pos:pos})
    ...
    add(item){
      ...
      let node= { id: item.path, label:label, color:color, type: 'file', group: "file", x:item.pos.x, y:item.pos.y} //, x: this.center.x, y:this.center.y}
      //  console.log(node)
    ...
    addDir(item){
      ...
      let node = {id: item.path, label:label,shape: 'box', color: color, type: 'folder', group: "folder", x:item.pos.x, y:item.pos.y} //, x: this.center.x, y:this.center.y}
      //  console.log(node)
      this.data.nodes.update([node])
    ...
    linkContainer(item){
      ...
      if (item.parent != undefined){
        let to = item.path
        //  splitted.pop()
        let from = item.parent //splitted.join(this.pathsep)
        let exist_edge =  this.data.edges.get({
          filter: function (e) {
  ...
  watch:{
    ...
    remoteResources(){
      ...
      let pos = {x: this.remoteResources.pos.x*1.5, y: this.remoteResources.pos.y*1.5}
      let root_node = {path: this.remoteResources.url, pos: pos}
      this.addDir(root_node)
      this.remoteResources.resources.forEach((r) => {
        let item = {path: r, parent: this.remoteResources.url, pos: pos}
        r.endsWith('/') ? this.addDir(item) : this.add(item)
        if (this.recursive == true && (r.endsWith('/') || r.endsWith('#me')) && !r.endsWith('2020/') && !r.endsWith('2021/')){
          this.$setCurrentThingUrl({url: r, pos:item.pos})
        }
      });
