...
export default {
  ...
  methods: {
    ...
    async load(s){
      ...
      let sources = []
      sources.push(s)
      let ouiz = new Ouiz(sources)
      let graphs = await ouiz.load()
      // let source = new Source(sources)
      // let graphs = await source.load()
      console.log(graphs)
      //send first nodes to vis
      this.$store.commit('ipgs/setGraphs', graphs)
      console.log("Now we can explode build edges")
      graphs = await ouiz.build()
      this.$store.commit('ipgs/setGraphs', graphs)
